...
def draw_line(screen, start, end, width, color):  #брезенхем алгоритм?
    x1 = start[0]
    y1 = start[1]   #начальная и конечная
    x2 = end[0]
    y2 = end[1]
    ...
    if dx > dy: #если линия ближе к горизонтальной
        if x1 > x2:
            x1, x2 = x2, x1
            y1, y2 = y2, y1
        ...
        if y1 > y2: #если линия ближе к вертикальной
            x1, x2 = x2, x1
            y1, y2 = y2, y1
...
def main():
    ...
    brush_png = pygame.transform.scale(pygame.image.load("image/brush.png"), (20, 20))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if mode == 'brush' or mode == 'rect' or mode == 'circle' or mode == 'triangle':
                    for i, col in enumerate(clr_rect_list, 0): #loop
                        if col.collidepoint(x, y):
                            color = clr_list[i]  #color check
